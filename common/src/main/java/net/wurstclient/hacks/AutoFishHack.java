...
public final class AutoFishHack extends Hack
    ...
    private final SliderSetting castRodDelay = new SliderSetting("Cast rod delay",
            "Amount of ticks to wait when it should cast the rod again.\n",
    ...
    private final CheckboxSetting shallowWaterWarning = new CheckboxSetting("Shallow water warning",
            "Shows a warning in chat if you're fishing in shallow water.\n",
            true);
    ...
    public AutoFishHack() {
        ...
        addSetting(reelInDelayRange);
        addSetting(castRodDelay);
        addSetting(randomizeReelInDelay);
    ...
    public void onUpdate() {
        ...
        // if fish bite, start timer
        if (fishBite) {
            biteReelInDelay--;

            if (biteReelInDelay <= 0) {
                rightClick();
                fishBite = false;
                castRodTimer = castRodDelay.getValueI();
                setReelInDelay();
    ...
    public void onReceivedPacket(PacketInputEvent event) {
        ...
        if (shallowWaterWarning.isChecked()) {
    ...
    private void setReelInDelay() {
        if (randomizeReelInDelay.isChecked()) {
            biteReelInDelay = ThreadLocalRandom.current().nextInt(0, Math.max(1, reelInDelayRange.getValueI()));
        } else {
            biteReelInDelay = reelInDelayRange.getValueI();
        }
    }
}